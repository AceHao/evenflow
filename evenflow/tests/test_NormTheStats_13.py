...
def reorder_3d_helper(array_3D, d1,d2,d3):
    ...
    for i in range(len(first)):
        out_array[i] = np.vstack((first[i], second[i], third[i]))
    return out_array
#Inputs
...
outputs = [InormByDist,TnormByDist,SigThreshInormByDist,SigThreshTnormByDist,Ic, Tc,TvsIzero,SigThreshTvsIzero,RelEnt,RelT,HXtNormByDist,IvsIzero,SigThreshIvsIzero ]

result = normthestats(nBinMat,I,T,SigThreshI,SigThreshT,meanShuffI,sigmaShuffI,meanShuffT,sigmaShuffT,HXt,HYw,HYf)
